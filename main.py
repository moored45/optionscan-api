...
# Verify price with live quote to catch stale data
quote = get_quote(sym)
if quote and quote["price"] > 0:
live_price = quote["price"]
bar_price = bars[-1]["c"]
# If bar price differs from live quote by more than 3%, data is stale
if abs(live_price - bar_price) / live_price > 0.03:
print(f"Stale data for {sym}: bar={bar_price} live={live_price}, skipping")
continue
price = live_price # Use live quote price
else:
price = bars[-1]["c"]
